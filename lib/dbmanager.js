...
class DBManager {
  ...
  async updateUser(name, week, year, fieldName, value, group) {
    ...
    if (group == "SKB") {
      await db.collection("users").doc(name).collection(year).doc(week).update(obj);
    } else {
      await db.collection("sapphire")
        .doc(name)
        .collection(year)
        .doc(week)
        .update(obj);
  ...
  async getUserNames() {
    ...
    for (let i = 0; i < snapshot.length; i++) {
      ...
      const thunderboltData = await thunderboltm.getUserDetails(namelist_link.data().namelist_link);
